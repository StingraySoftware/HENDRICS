...
class TestFullRun:
    ...
    def test_pds(self, data_kind, lombscargle):
        ...
        outA = os.path.join(
            self.datadir, "monol_testA_nustar_fpma_pds" + HEN_FILE_EXTENSION
        )
        outB = os.path.join(
            self.datadir, "monol_testB_nustar_fpmb_pds" + HEN_FILE_EXTENSION
        )
        if lombscargle:
    ...
    def test_cpds_rms_norm(self):
        """Test CPDS production."""
        command = (
            "{0} {1} -f 128 --save-dyn -k CPDS --save-all " "--norm rms -o {2}".format(
                self.lcA,
                self.lcB,
    ...
    def test_cpds_dtbig(self):
        """Test CPDS production."""
        command = (
            "{0} {1} -f 128 --save-dyn -k CPDS --save-all --norm " "frac -o {2}".format(
                self.lcA,
                self.lcB,
